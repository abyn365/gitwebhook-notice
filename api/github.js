...
function buildTelegramPayload(chatId, text, { silent = false, replyMarkup = null, disableWebPagePreview = true } = {}) {
  const payload = {
    chat_id: chatId,
    text,
...
async function sendToAllChats(botToken, chatIds, text, options = {}) {
  for (const chatId of chatIds) {
    try {
      await telegramRequest(botToken, "sendMessage", buildTelegramPayload(chatId, text, options));
    } catch (error) {
...
export default async function handler(req, res) {
  try {
    ...
    if (event === "watch") {
      ...
      await sendToAllChats(BOT_TOKEN, CHAT_IDS, text, {
