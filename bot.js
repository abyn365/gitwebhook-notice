...
async function getRedis() {
  ...
  if (!_redis) {
    ...
    _redis.on("error", e => console.error("Redis:", e));
  }
  if (!_redisPromise) {
    _redisPromise = (async () => {
      try {
      ...
      } catch { _redis = null; } finally { _redisPromise = null; }
    })();
  }
  await _redisPromise;
  return _redis;
...
async function setPending(userId, data) {
  ...
  else _memPending.set(userId, { data, exp: Date.now() + 300_000 });
}
...
async function clearPending(userId) {
  ...
  else _memPending.delete(userId);
}
